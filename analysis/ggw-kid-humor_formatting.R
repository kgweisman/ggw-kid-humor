...
files_run01 <- dir("/Users/kweisman/Documents/Research (Stanford)/Projects/GGW-kid/ggw-kid-humor/data/ggw-kids_kid_run_01_humor/")
...
for(i in 1:length(files_run01)) {
  # gather files
  f = files_run01[i]
  d_temp = read.csv(paste0("/Users/kweisman/Documents/Research (Stanford)/Projects/GGW-kid/ggw-kid-humor/data/ggw-kids_kid_run_01_humor/", files_run01[i]))
...
d_tidy = d_kid_run_01_raw %>%
  ...
  mutate(
    ...
    humorCat = factor(ifelse(humor > 0, "yes", "no"))
    ) %>%
...
write.csv(subset(d_tidy, run == "run01"), "/Users/kweisman/Documents/Research (Stanford)/Projects/GGW-kid/ggw-kid-humor/data/kid-run-01_2015-06-13_data_anonymized.csv")
